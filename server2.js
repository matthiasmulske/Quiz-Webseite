...
function updatePlayerAnswer(res, answerGiven, quizID, questionNumber, playerNumber) {
  const query = `UPDATE QuizQuestions SET AnswerPlayer${playerNumber} = ? WHERE QuizID = ? AND QuestionNumber = ?`;
  connection.query(query, [answerGiven, quizID, questionNumber], handleQueryResponse(res));
}
    ...
    connection.query(
      ...
      (err, results) => {
        ...
        batchQuery(questionQueries, questionsValues, res, connection, (err) => {
          if (err) {
            handleRollbackAndError(res, connection, "Error adding quiz questions", err);
            return;
          }
          ...
          const updateValues = [[q1, quizId], [q2, quizId], [q3, quizId]];

          batchQuery(updateQueries, updateValues, res, connection, (err) => {
            if (err) {
              handleRollbackAndError(res, connection, "Error updating quiz questions", err);
              return;
            }

            connection.commit((err) => {
              if (err) {
                handleRollbackAndError(res, connection, "Error committing transaction", err);
                return;
              }
...
function postComment(req, res) {
  ...
  const query = "INSERT INTO Comment (UserID, QuestionID, Text, CategoryID) VALUES (?, ?, ?, ?)";
  connection.query(query, [userID, questionID, text, categoryID], handleQueryResponse(res));
}
