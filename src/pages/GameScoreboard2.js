                    ...
                    <div key={index} className="row">
                            ...
                            <div className="row">
                                {chunk.map(question => (
                                    <div key={question.question_number} className="mb-3 col">
                                        <button className={`border w-50 btn ${getButtonColor(question.player1_answer, question.correct_answer)}`}
                                            onClick={() => handleButtonClick(question)}>{question.question_number}</button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className='col'>
                            <div className="row">
                                {chunk.map(question => (
                                    <div key={question.question_number} className="mb-3 col">
                                        <button className={`btn w-50 ${getButtonColor(question.player2_answer, question.correct_answer)}`}
                                            onClick={() => handleButtonClick(question)}>{question.question_number}</button>
                                    </div>
                                ))}
                            </div>
                        ...
                        {Object.keys(modalData?.answers || {}).map(key => {
                            return (
                                <li
                                    ...
                                    ${key === modalData?.correct_answer.toString() ? ' text-success fw-bold ' :
                                            (key === modalData?.player1_answer.toString() ? 'text-danger fw-bold ' : 'text-secondary')}`}
                                >
