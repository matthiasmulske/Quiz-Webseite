...
import { getQuestionsWithoutUser, updateUserForQuestion } from "../../api.js";
...
function QuestionTakeOver({ userID }) {
  ...
  async function getData() {
    let datas = await getQuestionsWithoutUser(
      domain.domain + "/getQuestionsWithoutUser",
      userID,
    );
    setData(datas);
  }
  ...
  async function handleTakeOver(userID, questionID) {
    ...
    updateUserForQuestion(
      domain.domain + "/updateUserForQuestion",
      userID,
      questionID,
    );
    setLoading(false);
  ...
  return (
    <div style={style.formContainer}>
      <p>
        Hier werden Fragen dargestellt, die ohne Besitzer sind. Fragen werden
        besitzerlos, falls der ursprüngliche Ersteller nicht innherhalb von drei
        Wochen auf Kommentare eingeht. Du kannst hier die Verantwortung für
        diese Fragen übernehemen!
      </p>
          ...
          <TableBody>
            {data.map((question, index) => (
              <TableRow key={index}>
                ...
                <TableCell>
                  <GameButton
                    onClick={() => handleTakeOver(userID, question.QuestionID)}
